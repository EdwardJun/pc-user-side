...
<template>
    <div class="scroll-num-wrap" :class="{active: navigatorStatus}" :style="{ width: (navigatorStatus?(cursors.length * 40 / 36 + 'rem') : (cursors.length * 40 + 'px'))}">
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            that.navigatorStatus = store.state.navigatorStatus
            that.splitNum()
        },
        methods: {
            splitNum () {
